const {cfg, file, System} = require('sm-utils');
...
async function waleExec(args, opts = {}) {
	...
	try {
		...
		logger.debug({label: 'wal-e'}, `\n${io.stdout}`);
		logger.info({label: 'wal-e'}, `\n${io.stderr}`);
		return io;
	}
	catch (err) {
		if (err.code === 'EPERM') {
 ...
 * @typedef {[string, import('moment').Moment, walFileName, string]} backupListItem
 */
 ...
 * @typedef {[string, import('moment').Moment, string, walFileName, string, walFileName, string]} backupListItemDetailed
 */
...
async function _getBackups(branch, host, detailed) {
	...
	try {
		...
		// @ts-ignore
		/** @type {T extends true ? backupListItemDetailed[] : backupListItem[]} */
 ...
 * @param {string} host
 */
async function getBackups(branch, host) {
	return _getBackups(branch, host, false);
}
...
async function getBackupsDetailed(branch, host) {
	return _getBackups(branch, host, true);
}
...
async function waleSpawn(args, opts, errFn = () => {}) {
	...
	return new Promise(async (resolve, reject) => {
		...
		cSpawn.stdout.on('data', data => logger.debug({label: 'wal-e', pid: cSpawn.pid}, `\n${data.toString()}`));

		cSpawn.stderr.on('data', (data) => {
			const info = data.toString();
			logger.info({label: 'wal-e', pid: cSpawn.pid}, `\n${info}`);
			err = info;
			errFn(info);
		});
		...
		cSpawn.on('close', (code) => {
			if (code !== 0) {
				// @ts-ignore
				if (code === 'EPERM') {
					logger.console({level: 'warn', pid: cSpawn.pid}, 'You don\'t have permission to run wal-e as \'postgres\' user, are you running this as root?');
				}
				else logger.console({level: 'warn', pid: cSpawn.pid}, 'POSTGRESQL MIGHT BE OFF, PLEASE RESTART IT');
		...
		cSpawn.on('exit', (code, signal) => {
			logger.log({lable: 'wal-e', pid: cSpawn.pid}, `Wal-e child process exited with code ${code}, ${signal}`);
		});
	});
}
...
async function waleDelete(branch, olderThan, total) {
	...
	try {
		let subComm = ['retain', String(olderThan)];
		if (olderThan === 0) {
			subComm = ['everything'];
		}
